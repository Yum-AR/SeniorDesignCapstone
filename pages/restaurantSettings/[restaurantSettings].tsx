...
import { collection, query, where, getDocs, QuerySnapshot, doc, updateDoc } from 'firebase/firestore'
import { db } from '../../firebase/clientApp'
import { useAuth } from '../../firebase/AuthContext'
import { useRouter } from 'next/router'
import NavBar from '../../components/Nav/NavBar'
import $ from 'jquery'
import DietaryIcon from '../../components/Icons/DietaryIcon'
export default function RestaurantSettings() {
    const [open, setOpen] = useState(false)
    const { currentUser } = useAuth()
    const router = useRouter()
    let restaurantID = router.query.restaurantSettings
  ...
  // ! implement appropiate model from Prisma
        ...
        let tuesday_start = document.getElementById('tuesday_start')
        let tuesday_end = document.getElementById('tuesday_end')

        let wednesday_start = document.getElementById('wednesday_start')
        let wednesday_end = document.getElementById('wednesday_end')

        let thursday_start = document.getElementById('thursday_start')
        let thursday_end = document.getElementById('thursday_end')

        let friday_start = document.getElementById('friday_start')
        let friday_end = document.getElementById('friday_end')

        let saturday_start = document.getElementById('saturday_start')
        let saturday_end = document.getElementById('saturday_end')

        let sunday_start = document.getElementById('sunday_start')
        let sunday_end = document.getElementById('sunday_end')
    ...
    const handleHoursForm = async (e) => {
        ...
        let mondayHours = `${hoursForm[0].value} - ${hoursForm[1].value}`
        let tuesdayHours = `${hoursForm[2].value} - ${hoursForm[3].value}`
        let wednesdayHours = `${hoursForm[4].value} - ${hoursForm[5].value}`
        let thursdayHours = `${hoursForm[6].value} - ${hoursForm[7].value}`
        let fridayHours = `${hoursForm[8].value} - ${hoursForm[9].value}`
        let saturdayHours = `${hoursForm[10].value} - ${hoursForm[11].value}`
        let sundayHours = `${hoursForm[12].value} - ${hoursForm[13].value}`
        let week = {
            Monday: mondayHours,
    ...
    const handleAddressForm = async (e) => {
        ...
        let street = document.getElementById('restaurantAddressStreet').value
        console.log(street)
        let city = document.getElementById('restaurantAddressCity').value
        let state = document.getElementById('restaurantAddressState').value
        let zip = document.getElementById('restaurantAddressZip').value
    ...
    const handleDietaryForm = async (e) => {
        ...
        let cuisines = []
        for (let index of dietaryForm) {
            cuisines.push(index.checked)
        }
        let dietaryValues = cuisines.slice(0, 5)
        let cuisineValues = cuisines.slice(5, -1)
    ...
    let addressNav = document.getElementById('addressNav')
    let addressDiv = document.getElementById('address-div')

    let dietaryNav = document.getElementById('dietaryNav')
    let dietaryDiv = document.getElementById('dietary-div')

    let generalInfoNav = document.getElementById('generalInfoNav')
    let generalDiv = document.getElementById('general-div')
    let genreNav = document.getElementById('genreNav')
    let imagesNav = document.getElementById('imagesNav')

    let divArr = [hoursDiv, addressDiv, dietaryDiv, generalDiv]
    const divSelector = (active) => {
        const nonActive = divArr.filter((divs) => divs != active)
        active.classList.remove('hidden')
        for (let divs of nonActive) {
