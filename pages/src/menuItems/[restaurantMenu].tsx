...
/* // TODO instead of fetching data after creating an item or deleting item,
we want to just visually update page from the component render cycle and only add one item.
another example is deleting item on page and deleting item but not fetching data after.*/
...
export default function restaurantMenu() {
  ...
  const { restaurantMenu } = router.query;
  let files = [];
  ...
  const toggleEditing = () => {
    ...
    const menuItemsListArr = document.getElementById(`menuItemsList`);
    if (isEditing === false) {
  ...
  const printQR = () => {
    ...
    const { qrCode0, qrCode1, qrCode2, qrCode3, qrCode4, qrCode5 } = prints;

    const qrCodeArr = [ qrCode0, qrCode1, qrCode2, qrCode3, qrCode4, qrCode5 ];

    const printDiv = document.createElement(`div`);

    printDiv.classList.add(`flex`, `flex-wrap`, `p-10`);

    for (const qrCode of qrCodeArr) {
  ...
  const createHeaderTextBox = () => {
    ...
    const addNewHeaderDiv = document.getElementById(`addNewHeaderDiv`);
    const newHeaderButton = document.getElementById(`newHeaderButton`);
    const submitNewHeader = document.createElement(`button`);
    submitNewHeader.style.display = ``;
    headerInputField.classList.add(`p-2`);
    submitNewHeader.classList.add(`inline`, `mt-2`, `bg-blue-500`,
      `hover:bg-blue700`, `text-white`, `text-sm`, `py-2`, `px-4`, `rounded`);
  ...
  const createMenuHeaders = async (headers, appendingDiv, dropdownID) => {
    const menuHeadersDiv = document.getElementById(appendingDiv);
    const menuHeadersDropdown = document.createElement(`select`);
    menuHeadersDropdown.setAttribute(`id`, dropdownID);
    menuHeadersDropdown.classList.add(`px-6`, `py-2.5`, `bg-gray-600`, `text-white`,
      `font-medium`, `text-sm`, `leading-tight`, `rounded`, `shadow-md`,
      `hover:bg-gray-700`, `hover:shadow-lg`, `text-center`, `pr-3`, `w-full`);
    for (const header of headers) {
      const menuHeadersOptions = document.createElement(`option`);
      menuHeadersOptions.setAttribute(`value`, header);
      menuHeadersOptions.classList.add(`dropdown-item`, `text-sm`, `py-2`, `px-4`,
        `font-normal`, `block`, `w-full`, `bg-transparent`, `text-white`);
  ...
  const filterItems = () => {
    const menuHeadersDropdown = document.getElementById(`menuHeadersSelect`);
    const menuHeaderIndex = menuHeadersDropdown?.options.selectedIndex;
    const filteredItems = [];
    for (const items of menuItemsUnfiltered) {
  ...
  if (isEditing) {
    const setHeadersDiv = document.getElementById(`setHeadersDiv`);
    const menuHeadersDropdown = document.getElementById(`menuSelect`);
  ...
  const formSubmit = () => {
    ...
    const menuHeaderID = setHeadersDiv?.childNodes[0].options.selectedIndex;
  ...
  const hideDirectory = () => {
    const directory = document.getElementById(`directoryListing`);
    const hideButton = document.getElementById(`hide-directory`);
    const showButton = document.getElementById(`show-directory`);
      ...
      <div className="flex-1 flex overflow-hidden">
        {active !== undefined
          ? <main className="flex-1 overflow-y-auto focus:outline-none ">
              {/* Breadcrumb */}
              <nav onClick={() => { hideDirectory(); }} id="hide-directory"
                className="flex items-start px-4 py-3 sm:px-6 lg:px-8 xl:hidden" aria-label="Breadcrumb">
              ...
              <nav onClick={() => { showDirectory(); }} id="show-directory"
                className="hidden items-start px-4 py-3 sm:px-6 lg:px-8 xl:hidden" aria-label="Breadcrumb">
                <a href="#" className="inline-flex items-center space-x-3 text-sm font-medium text-gray-900">
              ...
              <article>
                    ...
                    <div className="-mt-12 sm:-mt-16 sm:flex sm:items-end sm:space-x-5">
                      <div className="flex">
                        <Image
                          ...
                          src={active.thumbnailURL !== undefined
                            ? active.thumbnailURL
                      ...
                      <div className="mt-6 sm:flex-1 sm:min-w-0 sm:flex
                      sm:items-center sm:justify-end sm:space-x-6 sm:pb-1">

                        <div className="sm:hidden 2xl:block mt-6 min-w-0 flex-1">
                          <h1 className="text-2xl font-bold text-gray-900 truncate">
                            {active !== undefined ? active.menuItem : `Item Name`}</h1>
                        </div>
                        <div className="mt-6 flex flex-col justify-stretch
                        space-y-3 sm:flex-row sm:space-y-0 sm:space-x-4">
                          {isEditing ? <>
                            <span>
                              <button className='inline ml-3 bg-red-500 hover:bg-red-700
                               text-white font-bold py-2 px-4 rounded'
                              onClick={() => { setConfirmationModal(true); }}>Delete Item</button>
                ...
                <ConfirmationModal showConfirmationModal={showConfirmationModal}
                  setConfirmationModal={setConfirmationModal} deleteFoodItem={deleteFoodItem} />
                      ...
                      <div className="sm:col-span-2">
                        ...
                        {isEditing
                          ...
                          : <dd className="mt-1 text-sm text-gray-900">{
                              menuHeaders !== undefined ? menuHeaders[active.menuHeaderID] : `Item Name`}</dd>
                        }
                      </div>
                      <div className="sm:col-span-2">
                        <dt className="text-sm font-medium text-gray-500">Name</dt>
                        {isEditing
                          ? <input name="menuItem" id="menuItem" defaultValue={
                              active !== undefined ? active.menuItem : `Item Name`}
                            className="
                            w-[100%] shadow appearance-none border rounded py-2 px-3
                             text-gray-700 leading-tight focus:outline-none
                              focus:shadow-outline"
                            placeholder={active.menuItem} />
                          : <dd className="mt-1 text-sm text-gray-900">{
                              active !== undefined ? active.menuItem : `Item Name`}</dd>
                        }
                      </div>
                      <div className="sm:col-span-2">
                        <dt className="text-sm font-medium text-gray-500">Description</dt>
                        {isEditing
                          ? <input name="itemDescription" id="itemDescription"
                              defaultValue={
                                active !== undefined ? active.itemDescription : `Item Description`
                              }
                              className="w-[100%] shadow appearance-none border rounded py-2 px-3
                         text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                              placeholder={active.itemDescription} />
                          : <dd className="mt-1 text-sm text-gray-900">
                              {active !== undefined ? active.itemDescription
                                : `Item Description`}</dd>
                        }
                      </div>
                      <div className="sm:col-span-2">
                        ...
                        {isEditing
                          ? <input name="itemPrice" id="itemPrice"
                              defaultValue={active !== undefined ? active.itemPrice : `Item Price`}
                              className="w-[100%] shadow appearance-none border rounded py-2 px-3
                          text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                              placeholder={active.itemPrice} />
                          : <dd className="mt-1 text-sm text-gray-900">
                              {active !== undefined ? active.itemPrice : `Item Price`}
                            </dd>
                        }
                      </div>
                    </dl>
        ...
        <aside id="directoryListing"
          className="order-first xl:flex xl:flex-col
        flex-shrink-0 w-96 border-r border-gray-200">
          <div className="px-6 pt-6 pb-4">
            ...
            <button id='addNewItem' onClick={openModal}
              className='inline mt-2 bg-blue-500 hover:bg-blue-700
             text-white font-bold py-2 px-4 rounded'>New Item</button>
            <form className="mt-6 flex space-x-4" action="#">
              ...
              <div id="addNewHeaderDiv" className='flex flex-col'>
                <button onClick={filterItems} type="button" id="filterButton"
                  className='inline h-full bg-blue-500 mb-3 hover:bg-blue-700
                 text-white text-sm py-2 px-4 rounded'>Filter</button>
                <button type="button" id="newHeaderButton"
                  onClick={createHeaderTextBox}
                  className='inline h-full bg-blue-500 hover:bg-blue-700
                  text-white text-sm py-2 px-4 rounded'>New Header</button>
          ...
          <nav className="flex-1 min-h-0 overflow-y-auto" aria-label="Directory">
            {/* {menuItemArray.map((item)) => (
                <div key={} className="relative">
                    <div className="z-10 sticky top-0 border-t
                    border-b border-gray-200 bg-gray-50 px-6 py-1 text-sm font-medium text-gray-500">
                        <h3>A</h3>
                    </div> */}
            <ul role="list" id="menuItemsList" className="relative z-0 divide-y divide-gray-200">
              {menuItemArray.map((item) =>
                <li key={item.id} >
                  ...
                  space-x-3 hover:bg-gray-50 focus-within:ring-2 focus-within:ring-inset focus-within:ring-pink-500">
            ...
            <div className="m-1 mt-5 flex flex-col">
              <button className="bg-blue-500 self-center hover:bg-blue-700
               text-white font-bold py-2 w-[50%] px-4 rounded mb-2"
              onClick={printQR}>Print QR Code</button>
              <QRCode id="printCode" className="max-h-full self-center max-w-full mb-8"
                value={`yummr.io/restaurant/${restaurantMenu}`} />
            </div>
          </nav>
        </aside>
        {/* POPUP ELEMENT*/}
                ...
                <div className="inline-block w-full max-w-md p-6 my-8 overflow-hidden
                 text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl">
                  ...
                  <div className="mt-5 flex">
                    <form id="newItemForm" className='w-[100%]'>
                      <input placeholder="item title"
                        className="block w-[100%] mb-3 shadow appearance-none border
                       rounded py-2 px-3 text-gray-700 leading-tight focus:outline-none
                        focus:shadow-outline" name="newItemTitle" id="newItemTitle"></input>
                      <input placeholder="item description"
                        className="block w-[100%] mb-3 shadow appearance-none border
                       rounded  py-2 px-3 text-gray-700 leading-tight focus:outline-none
                       focus:shadow-outline" name="newItemDescription" id="newItemDescription"></input>
                      <input placeholder="item price"
                        className="block mb-3 w-[100%] shadow appearance-none border rounded
                        py-2 px-3 text-gray-700 leading-tight focus:outline-none
                         focus:shadow-outline" name="newItemPrice" id="newItemPrice" />
                    </form>
                  ...
                  <div className="mt-4 flex justify-center align-center">
                    <button
                      form="newItemForm"
                      className="px-4 py-2 text-sm font-medium text-blue-900 bg-blue-100
